...
function App() {
  ...
  const handleSettingsSave = async () => {
    try {
      const res = await fetch('http://localhost:3001/users/update-name', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: user.email, name: newName })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'İsim değiştirilemedi');
      ...
      <Router>
        ...
        <main className="main-content container">
          <Routes>
            ...
            <Route 
              path="/servis" 
              element={user ? <ServiceRequest /> : <Navigate to="/" replace />}
            />
          </Routes>
        </main>
        <Footer />
